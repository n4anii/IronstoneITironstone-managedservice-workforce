...
SET /P WingetContext=User or Machine (Machine should normally be used): 
ECHO install_silent_winget_psexec
IF "%_SANDBOX%"=="Yes" (
    ECHO Machine is detected as Sandbox
    ECHO Friendly warning: Winget is not supported in Sandbox. So this will fail!
    "%~dp0PsExec64.exe" \\localhost /accepteula /s "%~dp0Toolkit\Deploy-Application.exe" -DeploymentType Install -DeployMode Silent -AllowRebootPassThru -AppWizName %WingetAppWizName% -WingetID %WingetID% -WingetContext %WingetContext%
) ELSE (
    "%~dp0PsExec64.exe" /accepteula /s "%~dp0Toolkit\Deploy-Application.exe" -DeploymentType Install -DeployMode Silent -AllowRebootPassThru -AppWizName %WingetAppWizName% -WingetID %WingetID% -WingetContext %WingetContext%
)
TIMEOUT 10 >NUL
GOTO :MENU
...
IF "%_SANDBOX%"=="Yes" (
    ECHO Machine is detected as Sandbox
    ECHO Friendly warning: Winget is not supported in Sandbox. So this will fail!
    "%~dp0PsExec64.exe" \\localhost /accepteula /s "%~dp0Toolkit\Deploy-Application.exe" -DeploymentType Uninstall -DeployMode Silent -AllowRebootPassThru -AppWizName %WingetAppWizName% -WingetID %WingetID% -WingetContext %WingetContext%
) ELSE (
    "%~dp0PsExec64.exe" /accepteula /s "%~dp0Toolkit\Deploy-Application.exe" -DeploymentType Uninstall -DeployMode Silent -AllowRebootPassThru -AppWizName %WingetAppWizName% -WingetID %WingetID% -WingetContext %WingetContext%
)
TIMEOUT 10 >NUL
GOTO :MENU
