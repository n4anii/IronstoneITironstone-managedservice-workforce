<#
    .SYNOPSIS
        Configures screen saver timeout value, and enables secure lockscreen.
...
# Script Variables
$NameScript            = [string]$('Set-PowerConfigurationScreenSaverWait(10)')
$DeviceContext         = [bool]$($false)
$WriteToHKCUFromSystem = [bool]$($false)

# Script Settings
$ScreenSaverTimoutInMinutes = [uint16]$(10)
...
################################################
    ...
    $RegistryPath = [string]$('Registry::HKEY_CURRENT_USER\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\UserSetPowerConfigurationScreenSaverWait')

    # Continue only if it's not been done earlier
    if (Test-Path -Path $RegistryPath) {
        Write-Output -InputObject ('Registry {0} already set.' -f ($RegistryPath.Split('\')[-1]))
    }
    else {
        #Secure the lockscreen
        Set-ItemProperty -Path 'Registry::HKEY_CURRENT_USER\Control Panel\Desktop'  -Name 'ScreenSaverIsSecure' -Value 1 -Type 'DWord' -Force
        ...
        Function Set-ScreenSaverTimeout {
            ...
            $Seconds = [uint16]$($value * 60)
            $NullVar = [uint16]$(0)
            $SystemParamInfo::SystemParametersInfo(15, $Seconds, [REF]$NullVar, 2)
        }

        $Signature       = [string]$('[DllImport("user32.dll")]{0}public static extern bool SystemParametersInfo(int uAction, int uParam, ref int lpvParam, int flags );' -f ("`r`n"))
        $SystemParamInfo = Add-Type -MemberDefinition $Signature -Name 'ScreenSaver' -PassThru

        $Out = [bool]$(Set-ScreenSaverTimeout -Value $ScreenSaverTimoutInMinutes)
        

        # If success, write to registry that settings were set.
        if ($Out) {
            Write-Output -InputObject ('Creating registry path "{0}".' -f ($RegistryPath))
            $null = New-Item -Path ('{0}' -f ($RegistryPath)) -Force -ItemType 'Directory'
        }
        else {
