<#
    .NAME
        User_Uninstall-Bloatware.ps1

    .SYNOPSIS
        Removes specified pre-installed UWP apps considered bloatware from the users' profile.

    .DESCRIPTION
        Removes specified pre-installed UWP apps considered bloatware from the users' profile. This script MUST be deployed together with Device_Uninstall-Bloatware.ps1, else the apps will come back.
...
$NameScript            = [string] 'Uninstall-Bloatware'
...
################################################
    ...
    # Registry Path to Check / Write
    $RegistryPathCheck = [string]$('Registry::HKEY_CURRENT_USER\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\UserUninstallBloatware')

    # Only uninstall apps if it has not been attempted before
    if (Test-Path -Path $RegistryPathCheck) {
        Write-Output -InputObject ('Registry folder "{0}" already exist, meaning that the script has already run once. Will not proceed.' -f ($RegistryPathCheck.Split('\')[-1]))
    }
    else {
        $CurrentUserSID = [string]$([System.Security.Principal.WindowsIdentity]::GetCurrent().'User'.'Value')
        $AppsToRemove   = [string[]]@(
            ### Microsoft
            'Microsoft.BingFinance*',                           # Microsoft "MSN Money"
            'Microsoft.BingNews*',                              # Microsoft "News"
            'Microsoft.BingSports*',                            # Microsoft "MSN Sports"
            'Microsoft.BingWeather*',                           # Microsoft "Weather"
            #'Microsoft.Office.OneNote*',                       # Microsoft "OneNote" UWP App (The regular app soon to be gone from Office 365)
            #'Microsoft.OfficeOnline*',                         # Microsoft "Office Online"
            ...
            #'*ZuneVideo*',                                     # "Films & TV"
            ### Game developers
            'A278AB0D.*',                                       # March of Empires
            'flaregamesGmbH.*',                                 # Royal Revolt 2
            'GAMELOFTSA.*',                                     # Asphalt 8 Airborn
            ...
            '*Plex*',
            ### Social Media
            '9E2F88E3.*',                                       # Everything from Twitter
            'Facebook.*',                                       # Everything from Facebook
            '*Twitter*',                                        # Everything with "Twitter" in its' app name
            ### Other
            '*ActiproSoftwareLLC*',
            '*AutodeskSketchBook*',
            '*DrawboardPDF*',
        ...
        $AppxPackages = [System.Collections.ArrayList]$(Get-AppxPackage -User $CurrentUserSID | Where-Object -FilterScript {(-not($_.'NonRemovable')) -and (-not($_.'IsFramework'))})
        

        # Remove installed packages matching $AppsToRemove
        foreach ($App in $AppsToRemove) {
            $AppsPresentToRemove = [array]$($AppxPackages | Where-Object -Property 'Name' -like $App | Sort-Object -Property 'Name')
            Write-Output -InputObject ('AppxPackage(s) with name like "{0}" installed? {1}.' -f ($App,[string]$(if($AppsPresentToRemove.'Count' -ge 1){'True'}else{'False'})))
            if ($AppsPresentToRemove.'Count' -ge 1) {
                foreach ($AppRemove in $AppsPresentToRemove) {
                    $null = Remove-AppxPackage -Package $AppRemove.'PackageFullName' -User $CurrentUserSID -ErrorAction 'SilentlyContinue'
                    $Success = [bool]$($?)
                    Write-Output -InputObject ('{0}Uninstalling "{1}"... Success? {2}.' -f ("`t",$AppRemove.'Name',$Success.ToString()))
                    if ($Success){$null = $AppxPackages.Remove($AppRemove)}
                }
            }
        }
        ...
        $null = New-Item -Path $RegistryPathCheck -ItemType 'Directory' -Force
        Write-Output -InputObject ('Creating registry path "{0}". Success? {1}.' -f ($RegistryPathCheck,$?.ToString()))
    }
