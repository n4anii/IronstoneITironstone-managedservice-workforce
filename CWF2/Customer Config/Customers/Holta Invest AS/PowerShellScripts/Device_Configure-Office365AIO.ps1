<#
.SYNOPSIS
Auto configures Office 365 / 2016.

.DESCRIPTION
    Office General Configs
    - Disables First run prompts / videos
    - Enables MS XML format as standard
    - Enables diagnostics

    OneDrive for Business Configs
...
[string] $NameScript    = 'Configure-Office365AIO'
...
################################################
    ...
    #region    Get Current User + New PS Drive for HKU
        ...
        if (-not($Script:PathDirRootCU)){
            [string] $Script:PathDirRootCU = ('HKU:\{0}\' -f ([System.Security.Principal.NTAccount]::new((Get-Process -Name 'explorer' -IncludeUserName | Where-Object {$_.UserName -like 'AzureAD\*'} | Select-Object -ExpandProperty UserName -First 1)).Translate([System.Security.Principal.SecurityIdentifier]).Value))
    ...
    [PSCustomObject[]] $RegValues = @(
        # [PSCustomObject]@{Dir=[string]'directory';          Key=[string]'key';           Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | Common 
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\Common';                    Key=[string]'QMEnable';            Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKCU | Common \ General
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\Common\General';            Key=[string]'ShownFileFmtPrompt';  Val=[byte]1; Type=[string]'DWord'},
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\Common\General';            Key=[string]'ShownFirstRunOptin';  Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKCU | Common \ PTWatson
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\Common\PTWatson';           Key=[string]'PTWOptIn';            Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKCU | FirstRun
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\FirstRun';                  Key=[string]'BootedRTM';           Val=[byte]1; Type=[string]'DWord'},
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\FirstRun';                  Key=[string]'DisableMovie';        Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKCU | Registration
            [PSCustomObject]@{Dir=[string]'HKCU:\Software\Microsoft\Office\16.0\Registration';              Key=[string]'AcceptAllEulas';      Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKLM | Common \ General
            [PSCustomObject]@{Dir=[string]'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\16.0\Common\General';Key=[string]'ShownFileFmtPrompt';  Val=[byte]1; Type=[string]'DWord'},
            [PSCustomObject]@{Dir=[string]'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\16.0\Common\General';Key=[string]'ShownFirstRunOptin';  Val=[byte]1; Type=[string]'DWord'},
        # Office 365 / 2016 | HKLM | Registration
            [PSCustomObject]@{Dir=[string]'HKLM:\SOFTWARE\WOW6432Node\Microsoft\Office\16.0\Registration';  Key=[string]'AcceptAllEulas';      Val=[byte]1; Type=[string]'DWord'},
    ...
    #region    Set Registry Values
        foreach ($Item in $RegValues) {
            ...
            if ($TempPath -like 'HKCU:\*') {$TempPath = $TempPath.Replace('HKCU:\',$PathDirRootCU)}
