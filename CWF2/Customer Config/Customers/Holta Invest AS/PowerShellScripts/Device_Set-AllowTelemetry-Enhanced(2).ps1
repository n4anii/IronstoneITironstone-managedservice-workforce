...
[string] $NameScript    = 'Set-TelemetryLevel-Enhanced(2)'
...
################################################
    ...
    foreach ($Path in $Local:PathRegDirs) {
        foreach ($Key in $Local:NameRegKeys) {
            ...
            Write-Output -InputObject ('Set-ItemProperty -Path "{0}" -Name "{1}" -Value "{2}" -Type "DWord" -Force{3}    Success? {4}.' -f ($Path,$Key,$Local:TelemetryLevel,"`r`n",$?.ToString()))
        }
    }
