...
Try {
    ...
    #region    Main
        #region    Variables
            [string] $NameProgram       = 'MaxRMM'
            [string] $NameFileDownload  = 'agentmetieroec.exe'
            [string] $PathFileDownload  = ('{0}\Temp\{1}' -f ($env:windir,$NameFileDownload))
            [string] $PathDirInstall    = ('{0}\Advanced Monitoring Agent\winagent.exe' -f (${env:ProgramFiles(x86)}))
            [string] $PathFileLog       = ('{0}\Install-{1}-{2}.log' -f ($LogDirectory,$NameProgram,$NameFileDownload))          
            [string] $ArgsFileInstall   = ('/silent')
            [string] $URLFileDownload   = 'https://tilkoblet.no/metieroec/agentmetieroec.exe'
        #endregion Variables


        #region    Install
        If (Test-Path -Path $PathDirInstall) {
            Write-Verbose -Message ('"{0}" already Installed' -f ($NameProgram))
        }
        Else {
            Write-Verbose -Message ('WriteChanges = {0}' -f ($WriteChanges))
            Write-Verbose -Message ('Downloading "{0}".' -f ($NameFileDownload))
            If (Download-FileToDir -NameFileOut $NameFileDownload -DLURL $URLFileDownload -DLMethod 1) {                
                If ($WriteChanges) {
