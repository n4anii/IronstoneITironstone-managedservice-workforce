<#
    .NAME
        Device_Add-IETrustedSites(Microsoft).ps1


    .SYNOPSIS
        Adds common Microsoft domains to Internet Explorer Trusted sites, to make sure SSO, SharePoint, Office365 and other services functions properly.
    ...
    .AUTHOR
        Olav R. Birkeland @ Ironstone IT


    .CHANGELOG
        190122
        - Failproofed writing to HKCU from SYSTEM context by loading registry hive (%userprofile%\NTUSER.DAT) for not logged in users when running from system context
        181221
        - Failproofed writing to HKCU from SYSTEM context
        181219
        - Failproofed writing to HKCU from SYSTEM context
        181212
        - Added a whole lot of more domains
        - Simplified and failproofed writing to HKCU:\ from SYSTEM context
        180601
    ...
    .RESOURCES
        ...
        * Office 365 URLs and IP address ranges
          https://docs.microsoft.com/office365/enterprise/urls-and-ip-address-ranges
...
$NameScript            = [string] 'Add-IETrustedSites_Microsoft'
...
################################################
    ...
    # Domains - Adding 'lync.com' will translate to both 'https://*.lync.com' and 'https://www.*.lync.com'
    $Domains = [string[]]@(
        # Various
        'dynamics.com',
        'outlook.com',
        ...
        'microsoftazuread-sso.com'                   # autologon.microsoftazuread-sso.com
    )
    

    # Registry directories
    $Paths = [string[]]@(
        'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\ZoneMap\Domains\{0}',
        'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Internet Settings\ZoneMap\Domains\{0}\www',
        'Registry::HKEY_USERS\{0}\Software\Microsoft\Windows\CurrentVersion\Internet Settings\ZoneMap\Domains\{1}',
        'Registry::HKEY_USERS\{0}\Software\Microsoft\Windows\CurrentVersion\Internet Settings\ZoneMap\Domains\{1}\www'
    )
    ...
    foreach ($Path in $Paths) {
        foreach ($Domain in $Domains) {                      
            ...
            $PathDynamic = [string]$(if($Path -like 'Registry::HKEY_LOCAL_MACHINE\*'){$Path -f ($Domain)}else{$Path -f ($Script:StrIntuneUserSID,$Domain)})
            Write-Verbose -Message ('Path: "{0}".' -f ($PathDynamic))

            # Check if $TempPath exist, create it if not
            if (-not(Test-Path -Path $PathDynamic)){
                $null = New-Item -Path $PathDynamic -ItemType 'Directory' -Force -ErrorAction 'Stop'
                Write-Verbose -Message ('   Path did not exist. Successfully created it? {0}.' -f ($?.ToString()))
            }
            
            # Set-ItemProperty
            Set-ItemProperty -Path $PathDynamic -Name $Name -Value $Value -Type 'DWord' -Force -ErrorAction 'Stop'
