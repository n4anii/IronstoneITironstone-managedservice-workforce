...
Try {
    ...
    if (Test-Path -Path $RegistryPathCheck) {
        ...
        $AppsToRemove = [string[]]@(
            ...
            'Microsoft.People',
            'Microsoft.SkypeApp',
            'Microsoft.windowscommunicationsapps', 'Microsoft.BingFinance*',
            'Microsoft.BingNews*',
            'Microsoft.BingSports*',
            'Microsoft.BingWeather*',
            'Microsoft.SkypeApp*',
        ...
        $AppxPackages = [System.Collections.ArrayList]@(Get-AppxPackage -User ('{0}\{1}' -f ([System.Environment]::UserDomainName, [System.Environment]::UserName)) -PackageTypeFilter 'Bundle')
        ...
        foreach ($App in $AppsToRemove) {
            ...
            if ($CurrentApp.Count -eq 1) {
                ...
                $Local:Success = [bool]$($?)
                Write-Output -InputObject ('{0}Uninstalling... Success? {1}.' -f ("`t", $Local:Success.ToString()))
