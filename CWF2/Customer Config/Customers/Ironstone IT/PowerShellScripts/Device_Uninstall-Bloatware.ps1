<#
    .NAME
        Device_Uninstall-Bloatware.ps1

    .SYNOPSIS
        Removes specified pre-installed appxprovisionedpackages considered bloatware from the device.

    .DESCRIPTION
        Removes specified pre-installed appxprovisionedpackages considered bloatware from the device. This script MUST be deployed together with User_Uninstall-Bloatware.ps1, else the apps won't be removed from the user profile.
...
Try {
    ...
    $RegistryPathCheck = [string]$('Registry::HKEY_LOCAL_MACHINE\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\DeviceUninstallBloatware')
