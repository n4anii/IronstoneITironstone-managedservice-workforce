...
<#
    ...
    .DESCRIPTION
        If you need to delete the current files in addition to updating them you can run this script as a onetime script from scripts
...
$StorageAccountName = [string]$('istnoecw20ironsync')
$StorageAccountSASToken = [string]$('?sv=2021-06-08&ss=f&srt=sco&sp=rl&se=2032-08-16T19:23:27Z&st=2022-08-16T11:23:27Z&spr=https&sig=8mpYshFb9g2zABzPuAyGsqeFhdIe5qO32JPfpoOv2mE%3D')
...
Try {
    ...
    if (Test-Path -Path ("{0}{1}" -f ($PathDirIronSync), "\ironsync.csv")) {
        #Write in items you want to delete from the devices
        Remove-Item -Path "$env:appdata\Microsoft\Templates\Normal.dotm" -Force -ErrorAction SilentlyContinue
        Remove-Item -Path "$env:appdata\Microsoft\Templates\Word templates" -Recurse -Force -ErrorAction SilentlyContinue
        Remove-Item -Path "$env:appdata\Microsoft\Templates\PowerPoint templates" -Recurse -Force -ErrorAction SilentlyContinue
        Remove-Item -Path "$env:appdata\Microsoft\Teams\Backgrounds\Uploads" -Recurse -Force -ErrorAction SilentlyContinue
