...
### Settings - Share
$ShareLetter             = [string]$('K') # Change to desired driveletter (don't use double colon : )
$SharePath               = [string]$('\\SRV-MDT-01.bergans.local\MediaArchive') # Change to desired server / share path
$ShortCutTitle           = [string]$('MediaArchive') #this will be the name of the shortcut
$DesiredShortcutLocation = [string]$('{0}\Tools' -f ([Environment]::GetFolderPath('Desktop'))) # You can also use MyDocuments or any other valid input for the GetFolderPath function
...
$PersistantStorage       = [bool]$($false) # Persistant storage means Windows will remember the Share through reboots.
