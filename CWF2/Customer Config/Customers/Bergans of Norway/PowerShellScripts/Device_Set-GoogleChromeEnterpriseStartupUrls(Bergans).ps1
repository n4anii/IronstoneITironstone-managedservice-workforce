<#
    .NAME
        Device_Set-GoogleChromeEnterpriseStartupUrls(Customer).ps1

    .SYNOPSIS
        Sets Startup URLs for Google Chrome Enterprise.
...
# Customer Variables
$CustomerName          = [string]$('Bergans')
$ChromeStartupURLs     = [string[]]@('bergans.facebook.com','easitapp/zk/selfservice')
$ChromeRestoreOnStartup= [byte]$(4) # 0 = Not Configured, 1 = Restore the last session, 4 = Open a list of URLs, 5 = Open New Tab Page
...
################################################
    ...
    #>
        # Assets - URLs on top of the script
        ...
        foreach ($URL in $ChromeStartupURLs) {
            ...
            # Output success
            Write-Output -InputObject ('Added "{0}" as startup URL nr. {1}. Success? {2}.' -f ($URL,$C,$?.ToString()))          
        }
