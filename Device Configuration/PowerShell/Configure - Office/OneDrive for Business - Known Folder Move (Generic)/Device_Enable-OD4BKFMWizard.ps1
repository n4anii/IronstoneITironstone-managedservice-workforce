<#
Device_Enable-OD4BKFMWizard.ps1

.SYNAPSIS
Will enable OneDrive for Business Known Folder Move. Wizard.
...
[string] $NameScript    = 'Enable-OD4BKFMWizard'
...
#region    Assets
    ...
    [PSCustomObject[]] $RegValues = @(
        [PSCustomObject]@{Path=[string]$PathDirReg;Name=[string]'KFMOptInWithWizard';Value=[string]$TenantId;Type=[string]'String'}     
    )

    # Registry Values - Remove
    [PSCustomObject[]] $RegValuesRemove = @(
        [PSCustomObject]@{Path=[string]$PathDirReg;Name=[string]'KFMSilentOptIn';                }
        [PSCustomObject]@{Path=[string]$PathDirReg;Name=[string]'KFMSilentOptInWithNotification';}     
    )
#endregion Assets
