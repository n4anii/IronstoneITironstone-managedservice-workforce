...
# Variables
[String] $Local:NameScript         = 'IronTrigger'
[String] $Script:ScheduledTaskName = $Local:NameScript.Clone()
[String] $Script:DirInstall        = ('{0}\Program Files\IronstoneIT\{1}\' -f ($env:SystemDrive,$NameScript))
...
#region Functions
    ...
    #endregion Remove-ScheduledTask
#endregion Functions
...
If ($Script:IsEncrypted -and $Script:IsProtectionPassw -and $Script:IsBackupAAD -and $Script:IsBackupOD) {
    ...
    If (($Script:VolumeEncStatus -eq 'FullyEncrypted') -and ($Script:ArrayProtectionPasswords.Count -ge 1)) {
        ...
        # Files
        If (-not($Script:BoolRemoveFilesAfterSuccess)) {
            [String] $Local:LogDestPath = ('{0}{1}' -f ($Script:DirInstall,$Script:FileLog.Split('{\}')[-1]))
