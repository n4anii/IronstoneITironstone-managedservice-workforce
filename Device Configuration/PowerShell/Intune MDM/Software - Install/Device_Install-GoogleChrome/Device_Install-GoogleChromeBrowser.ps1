...
<#
    ...
    .NOTES
        ...
        Run from Intune
            Install - x86
                "%SystemRoot%\sysnative\WindowsPowerShell\v1.0\powershell.exe" -ExecutionPolicy "Bypass" -NoLogo -NonInteractive -NoProfile -WindowStyle "Hidden" -Command "& '.\Device_Install-GoogleChromeBrowser.ps1' -Architecture 'x86'; exit $LASTEXITCODE"
            Uninstall - x86
...
Param(
    ...
    [ValidateSet(IgnoreCase = $false,'x86')]
    [string] $Architecture
)
...
$ProductName            = [string]$('Google Chrome Browser {0}' -f ([string]$(if($Architecture -eq 'x86'){'x86'}else{'x64'})))
$ProcessName            = [string]$('chrome.exe')
$Uri                    = [string]$(if($Architecture -eq 'x86'){'https://dl.google.com/chrome/install/latest/chrome_installer.exe'}else{''})
$UriExpectedMinimumSize = [uint32]$(1MB)
$Destination            = [string]$('{0}\Temp\GoogleChromeBrowserSetup.exe' -f ($env:SystemRoot))
