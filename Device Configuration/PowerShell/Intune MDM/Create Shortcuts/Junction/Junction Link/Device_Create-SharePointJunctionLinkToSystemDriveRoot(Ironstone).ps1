...
$CustomerName          = 'Ironstone'

# Script Name & Settings
$NameScript            = [string]$('Create-SharePointJunctionLinkToSystemDriveRoot({0})' -f ($CustomerName))
...
################################################
    ...
    $PathDirSharePoint = [string]$('{0}\Users\{1}\{2}' -f ($SystemDriveRoot,$Script:StrIntuneUserName.Split('\')[-1],$CustomerName))
    

    # Exit if $PathDirSharePoint is malformed
    if ([string]::IsNullOrEmpty($PathDirSharePoint) -or $PathDirSharePoint -like '*\') {
        Throw 'ERROR: $PathDirSharePoint is empty or malformed.'
    }


    # Create PathFrom
    $PathFrom = [string]$('{0}\{1}-SharePoint' -f ($SystemDriveRoot,$CustomerName))
    

    # Output paths used for junction link
    Write-Output -InputObject ('Junction link from:     "{0}".' -f ($PathFrom))
    Write-Output -InputObject ('SharePoint folder path: "{0}".' -f ($PathDirSharePoint))


    # Remove if exist already
    if (Test-Path -Path $PathFrom) {        
        $null = [System.IO.Directory]::Delete($PathFrom)
        if ((-not$?) -or [bool]$(Test-Path -Path $PathFrom)) {
            Throw ('ERROR: Failed to delete "{0}".' -f ($PathFrom))
        }
    }


    # Create junction
    $null = New-Item -ItemType 'Junction' -Path $PathFrom -Value $PathDirSharePoint -ErrorAction 'Stop'
    if ($? -and [bool]$(Test-Path -Path $PathFrom)) {
