...
.SYNOPSIS
    Sets ComputerName to "<$ComputerNameBase><SerialNumber>"
...
# Customer Variables
$CustomerName     = [string] ''
$ComputerNameBase = [string] ''

# Script Name & Settings
$NameScript    = [string] ('Set-ComputerName({0})' -f ($CustomerName))
...
################################################
    ...
    #region    Check Customer Variables
        $CustomerName = $CustomerName.Trim()
        $ComputerNameBase = $ComputerNameBase.Trim()
        if ([string]::IsNullOrEmpty($CustomerName) -or [string]::IsNullOrEmpty($ComputerNameBase)) {
            Throw 'ERROR: $CustomerName and $ComputerNameBase cannot be empty.'
        }
    #endregion Check Customer Variables
    ...
    #region    Assets
        ...
        $ComputerNameOld  = [string]$($ComputerNameWmiObject | Select-Object -ExpandProperty 'Name' -ErrorAction 'SilentlyContinue').Trim()
    ...
    #region    Set ComputerName
        ...
        # If ComputerNameNew is not the same as ComputerNameOld = Change ComputerName
            ...
            # Set ComputerName
            $Success = [bool]$(if([byte]$(($ComputerNameWmiObject).Rename($ComputerNameNew) | Select-Object -ExpandProperty 'ReturnValue') -eq 0){$true}else{$false})
