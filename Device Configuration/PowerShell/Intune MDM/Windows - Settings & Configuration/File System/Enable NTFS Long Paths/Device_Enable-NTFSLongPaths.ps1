...
.DESCRIPTION
    * Enables NTFS Long Paths, which extends the default NTFS Path Character Limit of 260.
...
[string] $NameScript    = 'Enable-NTFSLongPaths'
...
[string] $Dir = 'HKLM:\SYSTEM\CurrentControlSet\Control\FileSystem'
[string] $Key = 'LongPathsEnabled'
[byte]   $Val = 1
Set-ItemProperty -Path $Dir -Name $Key -Value $Val -Type 'DWord' -Force
