<#
    .NAME
        Device_Set-WindowsFeatures.ps1
    ...
    .RESOURCES
        CMDLets
          * Get-WindowsCapability
            https://docs.microsoft.com/powershell/module/dism/get-windowscapability
          * Add-WindowsCapability
            https://docs.microsoft.com/powershell/module/dism/add-windowscapability
          * Get-WindowsOptionalFeature
            https://docs.microsoft.com/powershell/module/dism/get-windowsoptionalfeature
          * Disable-WindowsOptionalFeature
            https://docs.microsoft.com/powershell/module/dism/disable-windowsoptionalfeature
#>


# Script Variables
$NameScript            = [string] 'Set-WindowsFeatures'
...
################################################
    ...
    Write-Output -InputObject ('Windows Optional Feature "{0}" State: {1}' -f ($SMB1.'FeatureName',$SMB1.'State'))
        if ($SMB1.'State' -eq 'Enabled') {
            $null = Disable-WindowsOptionalFeature -Online -FeatureName $SMB1.FeatureName -NoRestart -ErrorAction 'SilentlyContinue'
            Write-Output -InputObject ('{0}Removing SMB1. Success? {1}.' -f ("`t",$?.ToString()))
        } 
        else {
            Write-Output -InputObject ('{0}SMB1 is already disabled. Nothing changed.' -f ("`t"))
        }
    #endregion Remove SMB1
    ...
    #region    Add .NET3.5
        ...
        if ($NET35.'State' -ne 'Installed') {
            $null = Add-WindowsCapability –Online -Name $NET35.'Name' -ErrorAction 'SilentlyContinue'
            Write-Output -InputObject ('{0}Adding .NET 3.5. Success? {1}' -f ("`t",$?.ToString()))
        }
        else {
            Write-Output -InputObject ('{0}.NET3.5 is already enabled. Nothing changed.' -f ("`t"))
        }
    #endregion Add .NET3.5
