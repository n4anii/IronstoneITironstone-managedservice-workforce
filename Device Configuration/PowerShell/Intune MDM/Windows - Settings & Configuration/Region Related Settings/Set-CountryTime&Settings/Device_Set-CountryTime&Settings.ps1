<# 
    .NAME
        Device_Set-CountryTime&Settings(nbNO).ps1
    ...
    .DESCRIPTION
        Norway (nbNO)
            Culture Name: nb-NO
            Locale ID:    1044
            Time Zone Id: W. Europe Standard Time
            Geo ID:       177
...
# Script Variables
$NameScript            = [string] 'Set-CountryTime&Settings(nbNO)'
...
################################################
    ...
    $RegistryPathCheck = [string]$('Registry::HKEY_LOCAL_MACHINE\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\Set-CountryTime&Settings')
    ...
    if (Test-Path -Path $RegistryPathCheck) {
        ...
        # Assets
        $TimeZoneIdWanted  = [string]$('W. Europe Standard Time')
        $GeoIdWanted       = [string]$('177')
        ...
        if ($TimeZoneCurrent.'Id' -ne $TimeZoneIdWanted) {
            ...
            Write-Output -InputObject ('{0}Changed to {1}. Success? {2}.' -f ("`t",$TimeZoneIdWanted,$?.ToString()))
        }


        # Windows Home Location / GeoId
        Write-Output -InputObject ('Current Home Location: {0}.' -f (($GeoIdCurrent = Get-WinHomeLocation).'HomeLocation'))
        if ([string]$($GeoIdCurrent.'GeoId') -ne [string]$($GeoIdWanted)) {
            Set-WinHomeLocation -GeoId $GeoIdWanted
            Write-Output -InputObject ('{0}Changed to {1}. Success? {2}.' -f ("`t",$GeoIdCurrent,$?.ToString()))
        }
