<# 
    .NAME
        User_Set-CountryTime&Settings(nbNO).ps1
...
################################################
    ...
    $RegistryPathCheck = [string]$('Registry::HKEY_CURRENT_USER\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\Set-CountryTime&Settings')
    ...
    if (Test-Path -Path $RegistryPathCheck) {
        ...
        # Assets
        $CultureNameWanted = [string]$('nb-NO')
        $LocaleLCIDWanted  = [string]$('1044')
        ...
        if ($CultureCurrent.'Name' -ne $CultureNameWanted) {
            ...
            Write-Output -InputObject ('{0}Changed to {1} Success? {2}..' -f ("`t",$CultureNameWanted,$?.ToString()))
        }
