<#
    .NAME
        User_Disable-KeyboardLayoutSwitchShortcut.ps1
...
$NameScript            = [string] 'Disable-KeyboardLayoutSwitchShortcut'
...
################################################
    ...
    $RegistryPathCheck = [string]$('Registry::HKEY_CURRENT_USER\SOFTWARE\IronstoneIT\Intune\DeviceConfiguration\{0}' -f ($NameScript))
    ...
    if (Test-Path -Path $RegistryPathCheck) {
        ...
        # Assets
        $Keys = [PSCustomObject[]]@(
            [PSCustomObject]@{Name=[string]'Hotkey';         Value=[string]'3';Type=[string]'String'},
            [PSCustomObject]@{Name=[string]'Language Hotkey';Value=[string]'3';Type=[string]'String'},
            [PSCustomObject]@{Name=[string]'Layout Hotkey';  Value=[string]'3';Type=[string]'String'}
        )

        # Set registry values
        foreach ($Key in $Keys) {
            $null = Set-ItemProperty -Path 'Registry::HKEY_CURRENT_USER\Keyboard Layout\Toggle' -Name $Key.'Name' -Value $Key.'Value' -Type $Key.'Type' -Force
