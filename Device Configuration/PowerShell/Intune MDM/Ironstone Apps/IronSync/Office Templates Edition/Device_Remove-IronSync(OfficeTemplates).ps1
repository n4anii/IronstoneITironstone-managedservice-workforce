...
$NameScript            = [string] 'Remove-IronSync(OfficeTemplates)'
...
################################################
    ...
    #region    Registry Values
        ...
        # Assets
        $RegPaths = [string[]]$(
            [string]$('Registry::HKEY_USERS\{0}\Software\Microsoft\Office\16.0\Excel\Options' -f ($Script:StrIntuneUserSID)),
            [string]$('Registry::HKEY_USERS\{0}\Software\Microsoft\Office\16.0\PowerPoint\Options' -f ($Script:StrIntuneUserSID)),
            [string]$('Registry::HKEY_USERS\{0}\Software\Microsoft\Office\16.0\Word\Options' -f ($Script:StrIntuneUserSID))
        )

        # Remove if item values exist
        foreach ($Path in $RegPaths) {
            if ([bool]$($null = Get-ItemProperty -Path $Path -Name 'PersonalTemplates' -ErrorAction 'SilentlyContinue';$?)){
                $null = Remove-ItemProperty -Path $Path -Name 'PersonalTemplates' -Force -ErrorAction 'Stop'
                Write-Verbose -Message ('"{0}\PersonalTemplates" deleted. Success? {1}.' -f ($Path,$?.ToString()))
            }
            else {
                Write-Verbose -Message ('"{0}\PersonalTemplates" does not exist.' -f ($Path))
            }
        }
    ...
    #region    Scheduled Tasks
        # Verbose
        Write-Verbose -Message ('# Scheduled Tasks')
        
        # Remove
        $null = Get-ScheduledTask | Where-Object {$_.'Author' -like 'Ironstone*' -and $_.'TaskName' -like '*IronSync*OfficeTemplates*'} | Unregister-ScheduledTask -Confirm:$false -ErrorAction 'Stop'
        Write-Verbose -Message ('Success.')
    #endregion Scheduled Tasks



    #region    Script Installation Files & Folders
        # Verbose
        Write-Verbose -Message ('# Script Installation Files & Folders')
        
        # Get Folder Path(s)
        $InstallPaths = [string[]]$(Get-ChildItem -Path ('{0}\IronstoneIT' -f ($env:ProgramW6432)) -Filter '*IronSync*OfficeTemplates*' -Directory -Recurse:$false | Select-Object -ExpandProperty 'FullName')

        # Remove
        foreach ($Path in $InstallPaths) {
            $null = Remove-Item -Path $Path -Recurse -Force -ErrorAction 'Stop'
            Write-Verbose -Message ('"{0}\*" deleted. Success? {1}.' -f ($Path,$?.ToString()))
        }

        # Verbose
        if ($InstallPaths.Count -le 0) {Write-Verbose -Message ('No directory found.')}
    #endregion Script Installation Files & Folders



    #region    Office Templates
        # Verbose
        Write-Verbose -Message ('# Office Templates')

        # Get Folder Path(s)
        $TemplatePaths = [string[]]$(Get-ChildItem -Path ('{0}\Users\Public' -f ($env:SystemDrive)) -Force -Filter '*OfficeTemplates*' -Directory -Recurse:$false | Select-Object -ExpandProperty 'FullName')


        # Check if we can remove files right now
        $CanDelete = [bool]$($true)
        :ForEachPath foreach ($Path in $TemplatePaths) {
            if (@(Get-ChildItem -Path $Path -Recurse -Force -File | Where-Object {$_.Name -Like '~$*' -and $_.Mode -eq '-a-h--'}).Count -ge 1) {
                Write-Verbose -Message ('Files in {0} are in use, can`t delete.' -f ($Path))
                $CanDelete = [bool]$($false)
                Break ForEachPath
            }
            else {
                Write-Verbose -Message ('No files in "{0}" are currently in use. Can Delete.' -f ($Path))
            }
        }
