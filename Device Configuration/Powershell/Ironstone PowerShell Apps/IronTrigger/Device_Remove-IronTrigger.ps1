...
.SYNOPSIS
    Removes files and scheduled task related to IronTrigger
...
#Change the app name
$AppName = 'Device_Remove-IronTrigger'
...
Try {
    ...
    Get-ScheduledTask | Where-Object {$_.TaskName -like '*trigger'} | ForEach-Object {
        Unregister-ScheduledTask -InputObject $_
    }

    # Remove Files
    Remove-Item -Path ('{0}\Program Files\IronstoneIT\IronTrigger\' -f ($env:SystemDrive)) -Recurse -Force -Confirm:$False
    ##########################################
