...
[string] $Script:WhatToConfig = 'User_Add-Registry-OneDrive-EnableADAL'
#endregion Variables
...
#region Main
    #region Set EnableADAL
    Write-DebugIfOn -In ("`r`n" + '### ' + $Script:WhatToConfig)
    [string] $Local:Dir = 'HKCU:\SOFTWARE\Microsoft\OneDrive'
    [string] $Local:Key = 'EnableADAL'
    [byte]   $Local:Val = 1
    Write-RegistryValueDword -Dir $Local:Dir -Key $Local:Key -Val $Local:Val
    #endregion Set EnableADAL
#endregion Main
