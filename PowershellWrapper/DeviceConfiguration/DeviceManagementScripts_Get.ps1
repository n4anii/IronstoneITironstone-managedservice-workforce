...
foreach ($PSScript in $PSScripts) {
    ...
    [array]$Assignments = $PSScript.groupAssignments.targetGroupId
    ...
    $Scriptcontent = (Get-DeviceManagementScripts -ScriptId $PSScript.id).scriptContent
    #$Scriptcontent = [System.Text.Encoding]::ASCII.GetString([System.Convert]::FromBase64String("$ScriptContent"))
    $PSScript | Add-Member -NotePropertyName AssignedToGroups -NotePropertyValue $AssignedToGroups -force
    $PSScript | Add-Member -NotePropertyName Scriptcontent -NotePropertyValue $Scriptcontent -force
