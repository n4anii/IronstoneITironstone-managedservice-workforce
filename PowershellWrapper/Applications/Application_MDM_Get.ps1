...
$Intune_Apps = Get-IntuneApplication

[array]$Output = $null
foreach ($app in $Intune_Apps) {
    
    
    [array]$Assignments = Get-ApplicationAssignment -ApplicationId $app.id
    ...
    $app | Add-Member -NotePropertyName AssignedToGroups -NotePropertyValue $AssignedToGroups -force
