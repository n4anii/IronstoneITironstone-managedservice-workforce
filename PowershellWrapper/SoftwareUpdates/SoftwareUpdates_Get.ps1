...
foreach ($WSUP in $WSUPs) {
    ...
    [array]$Assignments = $WSUP.groupAssignments.targetGroupId
    [array]$AssignedToGroups = $null
    if ($Assignments) {
        foreach ($Assignment in $Assignments) {
            #Write-Output "group"
            #Write-output -InputObject $group
            $GroupInfo = Get-AADGroup -id $Assignment
            $GroupInfoobject = @{
    ...
    $WSUP | Add-Member -NotePropertyName AssignedToGroups -NotePropertyValue $AssignedToGroups -force
    $WSUP | Add-Member -NotePropertyName UpdateType -NotePropertyValue 'Windows10' -force
    $Output += $WSUP 

}
foreach ($ISUP in $ISUPs) {

    [array]$Assignments = $ISUP.groupAssignments.targetGroupId
    [array]$AssignedToGroups = $null
    if ($Assignments) {
        foreach ($Assignment in $Assignments) {
            #Write-Output "group"
    ...
    $ISUP | Add-Member -NotePropertyName AssignedToGroups -NotePropertyValue $AssignedToGroups -force
    $ISUP | Add-Member -NotePropertyName UpdateType -NotePropertyValue 'iOS' -force
    $Output += $ISUP 
