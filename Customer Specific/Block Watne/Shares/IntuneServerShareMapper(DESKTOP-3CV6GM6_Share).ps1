...
$driveLetter             = [string]$('X') #change to desired driveletter (don't use double colon : )
$path                    = [string]$('\\DESKTOP-3CV6GM6\Share') #change to desired server / share path
$shortCutTitle           = [string]$('Share') #this will be the name of the shortcut
...
$desiredShortcutLocation = [string]$([Environment]::GetFolderPath('Desktop')) #you can also use MyDocuments or any other valid input for the GetFolderPath function


### START SCRIPT
$desiredMapScriptFolder  = [string]$('{0}\IronstoneIT\Intune\Scripts\SMB' -f ($env:LOCALAPPDATA))
$desiredMapScriptPath    = [string]$('{0}\SMBdriveMapper({1}).ps1' -f ($desiredMapScriptFolder,$path.Replace('\\','').Replace('$','').Replace('\',',')))

if (-not[System.IO.Directory]::Exists($desiredMapScriptFolder)) {
    $null = New-Item -Path $desiredMapScriptFolder -Type 'Directory' -Force
}
