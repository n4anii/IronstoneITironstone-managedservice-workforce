...
$driveMappingJson = '[{"Path":"\\\\home\\homedir","DriveLetter":"H","Label":null,"Id":0,"GroupFilter":null,"UserName":null,"Password":null},{"Path":"\\\\VOHV-FIL-01.VoiceNorge.No\\Voice-Data$","DriveLetter":"G","Label":null,"Id":2,"GroupFilter":"GS-ALL","UserName":null,"Password":null},{"Path":"\\\\vohv-fil-01.VoiceNorge.no\\Prosjekt","DriveLetter":"P","Label":null,"Id":3,"GroupFilter":"GS-PROSJEKT","UserName":null,"Password":null},{"Path":"\\\\vohv-fil-01.VoiceNorge.no\\Prosjekt","DriveLetter":"P","Label":null,"Id":4,"GroupFilter":"GS-Prosjekt-Ansattbilder","UserName":null,"Password":null},{"Path":"\\\\vohv-fil-01.VoiceNorge.no\\IKT$","DriveLetter":"L","Label":null,"Id":5,"GroupFilter":"GS-OKONOMI-IT","UserName":null,"Password":null},{"Path":"\\\\vohv-fil-01.VoiceNorge.no\\Prosjekt","DriveLetter":"P","Label":null,"Id":6,"GroupFilter":"GS-OKONOMI-IT","UserName":null,"Password":null},{"Path":"\\\\Grem3.gresvig.no\\m3economi","DriveLetter":"S","Label":null,"Id":7,"GroupFilter":"GS-OKONOMI-REGNSKAP-KASSEOPPGJOR","UserName":null,"Password":null},{"Path":"\\\\Grem3.gresvig.no\\m3economi","DriveLetter":"S","Label":null,"Id":8,"GroupFilter":"GS-OKONOMI-BUSINESSCONTROLLING","UserName":null,"Password":null},{"Path":"\\\\10.47.7.32\\f","DriveLetter":"F","Label":null,"Id":9,"GroupFilter":"Tekstil_appl_Arthur","UserName":null,"Password":null}]'
	...
	process {
		...
		try {
			...
			if ([string]::IsNullOrEmpty($env:USERDNSDOMAIN) -and [string]::IsNullOrEmpty($searchRoot)) {
				...
				$homeFolder = $searcher.FindOne().Properties.homedirectory
				$homeFolder = "$homeFolder"
				$homeFolder.Replace("\", "\\")
				$driveMappingConfig[0].Path = "$homeFolder"
...
if (-not (Test-RunningAsSystem)) {
	...
	foreach ($drive in $driveMappingConfig) {
		Write-Output $drive
		#if no username and pasword is given map as logged on user
		if ($drive.UserName -ne "null") {
			try {
		...
		#if a static username/passowrd is given map with theese credentials
		else {
			try {
				#check if variable in unc path exists, e.g. for $env:USERNAME -> resolving
				if ($drive.Path -match '\$env:') {
				...
				if ($process) {
					Write-Output "Mapping network drive $($drive.Path)"
					$password = ConvertTo-SecureString $drive.Password -AsPlainText -Force
					$cred = New-Object System.Management.Automation.PSCredential ($drive.UserName, $password)
					$null = New-PSDrive -PSProvider FileSystem -Name $drive.DriveLetter -Root $drive.Path -Description $drive.Label -Persist -Scope global -EA Stop -Credential $cred
							(New-Object -ComObject Shell.Application).NameSpace("$($drive.DriveLetter):").Self.Name = $drive.Label
				}
			}
			catch {
