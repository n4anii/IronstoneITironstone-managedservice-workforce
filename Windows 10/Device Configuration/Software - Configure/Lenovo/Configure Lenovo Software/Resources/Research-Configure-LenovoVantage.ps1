...
.SYNOPSIS
    Disables some elements in the Lenovo Vantage application.
...
.RESOURCES
    ...
    Lenovo Vantage Application for large Enterprises
    https://support.lenovo.com/solutions/hf003321

    Disable Lenovo System Update
    https://saniac.com/disable-lenovo-system-update-automatic-updating/
...
#region    Settings
    #region    ImController and Vantage Settings - All    
    ...
    #endregion ImController and Vantage Settings - All


    #region    Lenovo System Update Settings - All
        ...
        #region    Lenovo System Update Settings - Disable
            # Lenovo - System Update - Disable Auto Updates
                Write-Output -InputObject ('Lenovo - System Update - Disable Auto Updates')
                foreach ($Path in @($PathDirRegSystemUpdateUserSettings,$PathDirRegSystemUpdateUserSettingsScheduler)){if((-not([string]::IsNullOrEmpty($Path)))-and(-not(Test-Path -Path $Path))){$null = New-Item -Path $Path -ItemType 'Directory' -Force}}
                $null = Set-ItemProperty -Path $PathDirRegSystemUpdateUserSettingsScheduler -Name 'NotifyOptions' -Value 'NOTIFY' -Type 'String' -Force                            # How to notify the user.                   DOWNLOADANDINSTALL, DOWNLOAD, NOTIFY or DOWNLOADANDINSTALL -INCLUDEREBOOT
                $null = Set-ItemProperty -Path $PathDirRegSystemUpdateUserSettingsScheduler -Name 'SchedulerAbility' -Value 'NO' -Type 'String' -Force                             # Disable scheduled check for System Updates
                ...
                $null = Set-ItemProperty -Path $PathDirRegSystemUpdateUserSettingsGeneral -Name 'LaunchMode' -Value 'manual' -Type 'String' -Force                                 # How to launch LSU                         manual or auto
            ...
            # Lenovo - ImController and Vantage - System Update Plugin - Disable
                Write-Output -InputObject ('Lenovo - ImController and Vantage - System Update Plugin - Disable')
                $null = Set-ItemProperty -Path $PathDirRegImControllerSystemUpdate -Name 'Imc-Block' -Value 1 -Type 'DWord' -Force                                                 # Disable (1) or Enable (0) the System Update plugin of LSIF (Lenovo System Interface Foundation)
                $null = Set-ItemProperty -Path $PathDirRegVantage -Name 'E40B12CE-C5DD-4571-BBC6-7EA5879A8472' -Value 0 -Type 'DWord' -Force                                       # Hide (0) or Show (1) the System Update feature in the Vantage GUI
        #endregion Lenovo System Update Settings - Disable
    ...
    #endregion Lenovo System Update Settings - All
#endregion Settings
}
