...
# Version
$Version = [string] '201116'



# Script Settings
$DeviceContext         = [bool] $true
$IntuneWin32Wrapped    = [bool] $true
...
Try {
    ...
    if ($DeviceContext -and $Script:BoolIsCorrectUser -and $BoolWriteToHKCUFromSystem) {
        ...
        function Test-ValidIntuneSID {
            ...
            (Get-ChildItem -Path 'Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\ProfileList').'Name'.ForEach{$_.Split('\')[-1]} -contains $SID
        }
        # Help variables
        ...
        $SIDsProfileList = [string[]]($([string[]](Get-ChildItem -Path $PathProfileList -Recurse:$false).'Name').ForEach{$_.Split('\')[-1]}.Where{Test-ValidIntuneSID -SID $_})
        ...
        $IntuneUser = [PSCustomObject](
            [PSCustomObject[]]@(
                foreach ($x in [string[]]($([string[]](Get-ChildItem -Path 'Registry::HKEY_USERS' -Recurse:$false -ErrorAction 'SilentlyContinue').'Name').ForEach{$_.Split('\')[-1]}.Where{(Test-ValidIntuneSID -SID $_) -and $(Test-Path -Path ('Registry::HKEY_USERS\{0}\Software\IronstoneIT\Intune\UserInfo' -f ($_)))})) {
                    [PSCustomObject]@{
            ...
            [string]$(
                ...
                if (-not (Test-ValidIntuneSID -SID $Local:SID)) {
                    ...
                    $Local:SIDsFromRegistryAll = [string[]](
                        (Get-ChildItem -Path 'Registry::HKEY_USERS' -Recurse:$false -ErrorAction 'SilentlyContinue').'Name'.ForEach{
                            $_.Split('\')[-1]
                        }.Where{
...
#region    Assets
    ...
    #region    Assets & Variables
        ...
        # Prerequisites
        if ([string]::IsNullOrEmpty($ScriptNameNoun)) {
            $BoolScriptSuccess = $false; Throw 'ERROR: $ScriptNameNoun is Empty!'
            Break
        }
        else {
            Write-Verbose -Message ('{0}Prerequisites where met.' -f ("`t"))
        }
        ...
        #region    File Sync Folders                                                
            ...
            $PathDirSyncFilesSB     = [System.Management.Automation.ScriptBlock]{[string]$('{0}\IronSync' -f ($env:PUBLIC))}
    ...
    #region    Dynamic Assets
        ...
        $PathsToRemove = [string[]](
            $([string[]](                
                @($PathDirIronSync) +
                @(
                    Get-ChildItem -Path ('{0}\IronstoneIT' -f ($env:ProgramW6432)) -Directory -Force -Filter '*IronSync*OfficeTemplates*' -ErrorAction 'SilentlyContinue' | `
                    Select-Object -ExpandProperty 'FullName' | Where-Object -FilterScript {
                        $_ -notlike $PathDirIronSync
                    }
                ) +
                ('{0}\IronstoneIT\IronSync' -f ($env:ProgramW6432))
            )).Where{$_ -and [System.IO.Directory]::Exists($_)} | Sort-Object -Unique
        )
...
#region    Cleanup Previous Install           
    ...
    foreach ($Path in $PathsToRemove) {
        ...
        if ($Path -like ('{0}:*\\*' -f ($env:SystemDrive))) {
            Write-Error -Message 'ERROR: "{0}" is not a valid path.' -ErrorAction 'Stop'
        }
...
#region    Set Template folder to be hidden
    ...
    if ($ReadOnly) {
        ...
        (Get-Item -Path $PathDirSyncFiles -Force).'Attributes' = 'Hidden, ReadOnly, Directory'
...
#region    Create Scheduled Tasks                    
    ...
    #region    Create Scheduled Task running PS1 using PowerShell.exe - Every Day Every Hour When Powered On - Random Second to Not Flood Azure Storage Account
        ...
        $ScheduledTask.'Description' = ('{0}Runs a PowerShell script. {1}Execute: "{2}". {1}Arguments: "{3}".' -f (
            ...
            [string]($ScheduledTask.'Actions'.'Execute'),
            [string]($ScheduledTask.'Actions'.'Arguments')
        ))
...
Finally {
    ...
    if ($Script:BoolIsSystem -and $BoolWriteToHKCUFromSystem -and $RegistryLoadedProfiles.Where{$_}.'Count' -gt 0) {
