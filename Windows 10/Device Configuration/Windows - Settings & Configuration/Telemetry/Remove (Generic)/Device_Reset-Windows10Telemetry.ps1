<#
    .NAME
        Device_Reset-Windows10Telemetry

    .SYNAPSIS
        Deletes registry keys for Telemetry Level and CommercialID.
...
$NameScript            = [string] 'Reset-Windows10Telemetry'
...
################################################
    ...
    #region Variables
        ...
        #region    Static Paths
            ...
            $Names = [string[]]@('CommercialID','AllowDeviceNameInTelemetry','AllowTelemetry','AllowTelemetry_PolicyManager','MaxTelemetryAllowed')
        ...
        #region    Dynamic Paths
            ...
            $UserDir    = [string]$('Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\DataCollection\Users')
            $ChildItems = [string[]]@(Get-ChildItem -Path $UserDir | Select-Object -ExpandProperty 'Name' | ForEach-Object -Process {[string]$('Registry::{0}' -f ($_))})
            if ([byte]$([string[]]@($ChildItems).Count) -ge [byte]$(1)) {$Paths += @($ChildItems)}
        #endregion Dynamic Paths

        Write-Verbose -Message '# Paths to check:'
        $Paths | ForEach-Object {Write-Verbose -Message ('{0}{1}' -f ("`t",$_)) }
        Write-Verbose -Message '# Items to check in each path:'
        $Items | ForEach-Object {Write-Verbose -Message ('{0}{1}' -f ("`t",$_)) }
    #endregion Variables
    ...
    #region    Remove Registry Items
        ...
        foreach ($Path in $Paths) {
            foreach ($Name in $Names) {
                $Local:PathRegItem = [string]$('{0}\{1}' -f ($Path,$Name))
                ...
                if ((Get-ItemProperty -Path $Path -Name $Name -ErrorAction 'SilentlyContinue') -ne $null) {
                    Write-Verbose -Message ('   Remove-ItemProperty -Path "{0}" -Name "{1}"'-f ($Path,$Name))
                    if ($WriteChanges) {
                        ...
                        Write-Verbose -Message ('      $WriteChanges is $false, did not write changes.')
                    }
                }
                else {
