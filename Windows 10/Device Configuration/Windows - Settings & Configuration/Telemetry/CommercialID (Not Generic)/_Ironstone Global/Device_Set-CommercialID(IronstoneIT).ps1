<#
    .NAME
        Device_Set-CommercialID(Customer).ps1
    
    .SYNOPSIS
        Sets CommercialID in registry, to make sure Windows 10 Telemetry hits the correct Azure Log Analytics Workspace.
...
# Customer Variables
$CustomerName = [string] 'IronstoneIT'
$CommercialId = [string] '3e874c1f-1ac7-422a-b5d9-95c87a3af965'

# Script Name & Settings
$DeviceContext = [bool]   $true
$NameScript    = [string] ('Set-CommercialID({0})' -f ($CustomerName))
...
################################################
    ...
    $Name  = [string]$('CommercialID' )
    
    # Set CommercialId
    foreach ($Path in $Paths) {
        Write-Output -InputObject ('Set-ItemProperty -Path "{0}" -Name "{1}" -Value "{2}" -Type "String" -Force' -f ($Path,$Name,$CommercialId))
        Set-ItemProperty -Path $Path -Name $Name -Value $CommercialId -Type 'String' -Force -ErrorAction 'Stop'
