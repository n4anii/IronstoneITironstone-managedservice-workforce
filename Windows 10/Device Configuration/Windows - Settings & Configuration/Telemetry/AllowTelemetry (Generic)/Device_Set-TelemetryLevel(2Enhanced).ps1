<#
    .NAME
        Device_Set-TelemetryLevel(2Enhanced).ps1
...
$NameScript            = [string] 'Set-TelemetryLevel(2Enhanced)'
...
################################################
    ...
    $PathRegDirs    = [string[]]@('Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Policies\Microsoft\Windows\DataCollection','Registry::HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Policies\DataCollection')
    $NameRegKeys    = [string[]]@('AllowTelemetry','AllowTelemetry_PolicyManager')    
    
    # Set Registry Keys
    foreach ($Path in $PathRegDirs) {
        # Create Path If Not Exist
        if (-not(Test-Path -Path $Path)){$null = New-Item -Path $Path -ItemType 'Directory' -Force -ErrorAction 'Stop'}    
        # Set Registry Values
        foreach ($Key in $NameRegKeys) {
            Write-Output -InputObject ('Set-ItemProperty -Path "{0}" -Name "{1}" -Value "{2}" -Type "DWord" -Force' -f ($Path,$Key,$TelemetryLevel))
            $null = Set-ItemProperty -Path $Path -Name $Key -Value $TelemetryLevel -Type 'DWord' -Force -ErrorAction 'Stop'
