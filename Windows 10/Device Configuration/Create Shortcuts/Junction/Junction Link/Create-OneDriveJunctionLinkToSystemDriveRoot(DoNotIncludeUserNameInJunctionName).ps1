...
$IncludeUserNameInJunctionLinkName = [bool]$($false)

# Script Name & Settings
$NameScript            = [string]$('Create-OneDriveJunctionLinkToSystemDriveRoot({0}IncludeUserNameInJunctionName)' -f ([string]$(if(-not($IncludeUserNameInJunctionLinkName)){'DoNot'})))
...
################################################
    ...
    # Assets
    $SystemDriveRoot = [string]$([System.Environment]::GetEnvironmentVariable('SystemDrive'))
    $PathDirOneDrive = [string]$(Get-ItemProperty -Path ('Registry::HKEY_USERS\{0}\Environment' -f ($Script:StrIntuneUserSID)) -Name 'OneDriveCommercial' | Select-Object -ExpandProperty 'OneDriveCommercial')

    
    # Exit if $PathDirOneDrive is empty or malformed
    if ([string]::IsNullOrEmpty($PathDirOneDrive) -or $PathDirOneDrive -like '*\') {
        Throw 'ERROR: $PathDirOneDrive is empty or does not exist.'
    }


    # Create PathFrom
    $PathFrom = [string]$(
        if ($IncludeUserNameInJunctionLinkName) {
            [string]$('{0}\{1}-{2}' -f ($SystemDriveRoot,$Script:StrIntuneUserName.Split('\')[-1],$PathDirOneDrive.Split('\')[-1].Replace(' ','')))
        }
        else{
            [string]$('{0}\OneDrive' -f ($SystemDriveRoot))
        }
    )
    

    # Output paths used for junction link
    Write-Output -InputObject ('Junction link from:   "{0}".' -f ($PathFrom))
    Write-Output -InputObject ('OneDrive folder path: "{0}".' -f ($PathDirOneDrive))


    # Remove if exist already
    if (Test-Path -Path $PathFrom) {        
        $null = [System.IO.Directory]::Delete($PathFrom)
        if ((-not$?) -or [bool]$(Test-Path -Path $Path)){Throw ('ERROR: Failed to delete "{0}".' -f ($PathFrom))}
    }


    # Create junction
    $null = New-Item -ItemType 'Junction' -Path $PathFrom -Value $PathDirOneDrive -ErrorAction 'Stop'
    if ($? -and [bool]$(Test-Path -Path $Path)) {
        Write-Output -InputObject ('Successfully created junction.')
    }
    else {
        Write-Output -InputObject ('Failed to create junction.')
    }
