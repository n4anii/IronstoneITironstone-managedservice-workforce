...
[String] $EnterpriseMAKKey = 'D2DB6-7NFB3-GV9K7-Y9BBD-M7V2F'
#endregion Variables
    ...
    If ($ActStatus.Status -like 'Licensed') {
        ...
        If ($WindowsEdition -Like '*enterprise*') {
            If (!($ReadOnly)) {
                ...
                $null = $ActivationService.InstallProductKey($EnterpriseMAKKey)
                $null = $ActivationService.RefreshLicenseStatus()
                $ActStatusNow = Get-ActivationStatus -HostName $env:COMPUTERNAME
                If ($ActStatusNow.Status -like 'Licensed') {
